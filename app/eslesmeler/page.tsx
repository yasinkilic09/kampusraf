...
function getScorePercent(score: number) {
  const percent = Math.round(Math.min(score * 18, 100));
      ...
      <section className="mx-auto max-w-7xl px-4 py-6 md:px-6 md:py-10">
        <div className="rounded-[1.7rem] bg-[#2E7D5B] p-6 text-white shadow-2xl shadow-[#2E7D5B]/20 md:rounded-[2rem] md:p-12">
          ...
          <p className="mt-4 max-w-2xl text-sm leading-7 text-white/75 md:text-base">
            Aranan kitaplar ile sisteme eklenen kitaplar arasında bulunan
            otomatik eşleşmeler burada görünür.
          </p>
        </div>
        ...
        <div className="mt-6 grid gap-3 md:mt-8 md:grid-cols-3 md:gap-5">
          ...
          <div className="rounded-[1.7rem] bg-white p-5 shadow-sm md:rounded-[2rem] md:p-7">
            <p className="text-sm font-bold text-slate-500">Benim Aramalarıma Gelen</p>
          ...
          <div className="rounded-[1.7rem] bg-white p-5 shadow-sm md:rounded-[2rem] md:p-7">
            <p className="text-sm font-bold text-slate-500">Benim Kitaplarımla Eşleşen</p>
            <p className="mt-3 text-4xl font-black text-[#2E7D5B]">
        ...
        {!error && matches.length === 0 ? (
          ...
          <div className="mt-6 space-y-4 md:mt-8 md:space-y-5">
            {matches.map((match) => {
              ...
              const bookTitle =
                userBook?.custom_title || relatedBook?.title || "Kitap bilgisi yok";
              const bookAuthor =
                userBook?.custom_author || relatedBook?.author || "Yazar bilgisi yok";
              ...
              return (
                <article
  key={match.id}
  className="rounded-[1.7rem] bg-white p-4 shadow-sm transition hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-900/5 md:rounded-[2rem] md:p-6"
>
                  <div className="grid gap-4 md:gap-6 lg:grid-cols-[0.75fr_1.25fr]">
                    <div className="flex gap-3 md:gap-4">
                      <div className="flex h-24 w-16 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-[#FAF7F0] text-2xl md:h-32 md:w-24 md:text-3xl">
                        {image ? (
  ...
  <span className="rounded-full bg-[#F59E0B]/10 px-3 py-1 text-xs font-black text-[#F59E0B]">
                          %{getScorePercent(match.match_score)} uyum
                        </span>
  ...
  {request?.title || "Arama kaydı"}
                        ...
                        <p className="mt-4 text-sm font-semibold text-slate-500">
                          Karşı taraf:{" "}
                          <span className="font-black text-[#2E7D5B]">
...
>
  Kitabı Gör
                        ...
                        <form action={startMatchConversationAction} className="w-full sm:w-auto">
